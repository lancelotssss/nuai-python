...
export default function OfficerDashboardCreatePost({ onDone, onCancel }) {
  ...
  function hasUnsavedChanges() {
    const hasTextChanges = cleanText(headerText) || cleanText(contentText);

    const hasLinkChanges = links.some(
      (link) => cleanText(link.label) || cleanText(link.url),
    );

    const hasImageChanges = files.length > 0;

    return Boolean(hasTextChanges || hasLinkChanges || hasImageChanges);
  }

  const canSaveDraft = hasUnsavedChanges();

  function leaveCreatePostPage() {
    ...
    navigate("/alumni-officer/posts", { replace: true });
  }

  function requestExit(exitAction = null) {
    if (loadingRef.current) return;

    const action = exitAction || leaveCreatePostPage;
  ...
  function handleConfirmExitWithoutSaving() {
    const action = pendingExitAction || leaveCreatePostPage;
  ...
  async function handleSaveDraftAndExit() {
    ...
    const saved = await handleSaveDraft({ skipRedirect: true });
  ...
  }, [headerText, contentText, links, files]);

  useEffect(() => {
    function handlePostFormExitRequest(event) {
      if (loadingRef.current || showExitConfirm) return;
      ...
      setPendingExitAction(() => event.detail?.proceed || leaveCreatePostPage);
      setShowExitConfirm(true);
    }

    window.addEventListener(
      "officer-post-form-exit-request",
      handlePostFormExitRequest,
    );
  ...
  useEffect(() => {
    ...
    function handlePopState() {
      ...
      window.history.pushState(guardState, "", window.location.href);
      setPendingExitAction(() => leaveCreatePostPage);
      setShowExitConfirm(true);
    }
  ...
  function handleImageSelection(e) {
    ...
    if (deduped.length > MAX_IMAGE_COUNT) {
    ...
    setFiles(deduped.slice(0, MAX_IMAGE_COUNT));
  ...
  const selectedSummary = useMemo(() => {
    return cleanText(headerText) || "No post header yet";
  }, [headerText]);
  ...
  async function handleSubmit() {
    ...
    if (!cleanText(headerText)) return;
    if (!cleanText(contentText)) return;
    ...
    try {
      ...
      if (uploadedURLs.length > 0) {
        await updateDoc(doc(db, "newsPosts", docRef.id), {
      ...
      logAudit({
        action: "POST_CREATED",
        details: `Post Created ${docRef.id} by ${
  ...
  async function handleSaveDraft(options = {}) {
    ...
    try {
      ...
      logAudit({
        ...
        details: `Post Draft Created ${docRef.id} by ${
          auth.currentUser?.uid || "Unknown"
        }`,
      }).catch(() => {});
    ...
    switch (currentStep) {
      case 1:
        return (
          <FormShell
            ...
            helper="Write the main post header and content for the alumni news feed."
            icon={FileText}
          >
      ...
      case 2:
        return (
          <FormShell
            title="Step 2: Links"
            helper="Add optional links related to this post."
            icon={Link2}
            actions={
              <button
                type="button"
            ...
            <div className="space-y-5">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">
                        {files.length > 0
                          ? `${files.length}/${MAX_IMAGE_COUNT} image(s) selected`
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      These will be uploaded when you create the post.
                    </p>
                  </div>
              ...
              {!imagePreviews.length ? (
                <div className="rounded-2xl border border-dashed border-border bg-muted/20 px-5 py-8 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Choose files to preview them here before creating the post.
                  </p>
                </div>
              ) : null}
            </div>
      ...
      case 4:
        return (
          <FormShell
            ...
            helper="Review how your post will appear before publishing it to the alumni news feed."
            icon={ShieldCheck}
          >
            <div className="space-y-5">
              <PostFeedPreview
  ...
  return (
    <>
      ...
      <AlertDialog open={showDraftConfirm} onOpenChange={setShowDraftConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Save Post as Draft?</AlertDialogTitle>
            <AlertDialogDescription>
              This will save the post as a draft. You can continue editing and
              publish it later.
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <AlertDialog open={showExitConfirm} onOpenChange={setShowExitConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Your current post changes have not been saved. You can save this
              post as a draft before leaving.
            </AlertDialogDescription>
          </AlertDialogHeader>
