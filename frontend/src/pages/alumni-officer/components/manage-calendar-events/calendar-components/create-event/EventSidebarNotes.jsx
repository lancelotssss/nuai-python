...
import { SectionHeader } from "../../../manage-perks-discounts/perk-components/PerksFormSectionCard";
...
export default function EventSidebarNotes({ form }) {
  const displayCategory =
    form?.category === "others" ? form?.customCategory : form?.category;

  const previewTitle = form?.title?.trim() || "Untitled event";
  const previewContent = form?.description?.trim() || "No description yet.";
  ...
  const postingValue =
    Array.isArray(form?.postedOn) && form.postedOn.length > 0
      ? form.postedOn.join(", ")
      : "calendar";
      ...
      <CardContent className="p-0">
        <div className="border-b border-border px-5 py-5">
          <SectionHeader
            icon={MapPin}
            title="Preview Notes"
            helper="What this event form will save."
          />
        </div>
        ...
        <div className="space-y-3 p-5">
          ...
          <InfoCard icon={CalendarDays} label="Schedule" value={schedule} />

          <InfoCard
            icon={MapPin}
            label="Location"
            value={form?.location?.trim() || "Not set"}
          />

          <InfoCard
            icon={Tag}
            label="Category"
            value={displayCategory || "Not set"}
          />
